...
import {
  LayoutDashboard, Monitor, Camera, BookOpen,
  LogOut, LogIn, ChevronLeft, ChevronRight
} from "lucide-react";
import { useState } from "react";

const navItems = [
  { id: "monitor", label: "Monitoring Kiosk", icon: Monitor },
  { id: "sessions", label: "Sesi Foto", icon: Camera },
  { id: "panduan", label: "Panduan Operator", icon: BookOpen },
];
...
const OperatorSidebar = ({ activeSection, onSectionChange }: OperatorSidebarProps) => {
  ...
  return (
    <div className={`${collapsed ? "w-16" : "w-56"} transition-all duration-300 border-r border-border bg-card/50 flex flex-col shrink-0`}>
      <div className="p-4 border-b border-border flex items-center justify-between">
        {!collapsed && (
          <div>
            <h2 className="text-sm font-bold text-foreground">Operator</h2>

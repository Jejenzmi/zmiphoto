...
import {
  LayoutDashboard, Monitor, Camera, DollarSign, PieChart,
  FileText, LogOut, LogIn, ChevronLeft, ChevronRight
} from "lucide-react";
import { useState } from "react";

const navItems = [
  { id: "overview", label: "Ringkasan", icon: LayoutDashboard },
  { id: "kiosks", label: "Status Kiosk", icon: Monitor },
...
const VenueSidebar = ({ activeSection, onSectionChange }: VenueSidebarProps) => {
  ...
  return (
    <div className={`${collapsed ? "w-16" : "w-56"} transition-all duration-300 border-r border-border bg-card/50 flex flex-col shrink-0`}>
      <div className="p-4 border-b border-border flex items-center justify-between">
        {!collapsed && (
          <div>
            <h2 className="text-sm font-bold text-foreground">Venue</h2>
